...
void cg::renderer::dx12_renderer::load_pipeline()
{
	...
	// create render target view for each RT
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	THROW_IF_FAILED(device->CreateGraphicsPipelineState(
		&pso_descriptor, IID_PPV_ARGS(&pipeline_state)));
	...
	THROW_IF_FAILED(device->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(64 * 1024), D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr, IID_PPV_ARGS(&constant_buffer)));
